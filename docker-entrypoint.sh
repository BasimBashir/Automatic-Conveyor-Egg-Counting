...
# Counting runs through ultralytics ObjectCounter, which infers at imgsz 640 by
# default (matching a 640x480 sub-stream). The engine is built at TRT_IMGSZ
# (default 640) so the .engine's fixed input shape matches what ObjectCounter
# feeds it. Override TRT_IMGSZ if you run a higher-resolution main stream.
...
exec uvicorn app.main:app --host 0.0.0.0 --port 5590 --workers 1
